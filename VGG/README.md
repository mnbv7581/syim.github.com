...
즉, 평균은0, 표준편차는 1인 분포를 만드는 과정이라고 생각하면 되며(좀더 디테일게 들어가면 다른 변수들이 등장하긴 함)
논문 내용을 보면 분산을 구한 다음 분산에 epsilon이라는 미세한 값을 더 한것의 제곱근으로 나누는데 결국 표준편차를 나누는 것이라고
생각하면 됩니다. 그렇게 [표준값, 표준점수]를 구하게 됩니다(x의 값이 평균에서 얼만큼 떨어져 있는지를 계산하게 됨)
...
50000장의 트레이닝 셋과 10000장의 테스트 셋에 대한 결과.
학습을 반복할 때 마다 Test set과 Valid set의 정확도가 올라간 것을 확인했습니다.
좀 더 정확도를 끌어올리려면 하이퍼파라미터를 수정하여 반복학습을 진행해야 할 거같습니다.
