...
def bottleneck_layer(input, layer_name, isTraining):
    ...
    x = tf.nn.relu(x)
    x = tf.layers.conv2d(inputs = x, filters = K * 4, kernel_size = [3, 3], strides = 1, padding = 'SAME', name = layer_name + '_conv_2')
    x = tf.layers.dropout(inputs = x, rate = DROP_OUT, training = isTraining)
    print(x)
...
def DenseNet(input, training_flag):
    ...
    x = tf.layers.conv2d(inputs = input, filters = K * 2, kernel_size = [7, 7], strides = 2, padding = 'SAME', name = 'dense_0_conv_1')
    #x = MaxPooling(filter_shape = (3, 3), strides = 2, name = 'pool1')(x)
    print(x)
